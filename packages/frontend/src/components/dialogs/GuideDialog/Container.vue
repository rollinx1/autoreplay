...
await send(request);`,
  ...
  },
  {
    ...
    description: "Access original response headers",
    code: `if (response !== undefined) {
  ...
  >
    <template #header>
      <div class="flex items-center justify-between w-full">
        <span class="font-semibold text-lg">Check Script Guide</span>
        <Button
          icon="fas fa-times"
          text
          severity="secondary"
    ...
    <div class="flex flex-col gap-6 overflow-y-auto pr-1">
      ...
      >
        <h3
          class="text-xs font-bold text-secondary-400 uppercase tracking-wider border-b border-surface-700 pb-1 sticky top-0 bg-surface-900 z-10"
        >
          {{ group.category }}
        </h3>
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <Button
              :icon="
                copiedIndex === pattern.index ? 'fas fa-check' : 'fas fa-copy'
              "
              :severity="
