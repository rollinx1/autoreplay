...
const { fetchCallbackConfigs, deleteCallbackConfig } = useSettings();
const { openAddCallback } = useDialog();

onMounted(() => {
  void fetchCallbackConfigs();
});

const onDelete = async (id: string) => {
  await deleteCallbackConfig(id);
};
</script>
  ...
  <Card class="h-full">
    <template #content>
      <div class="flex flex-col h-full">
        ...
        >
          <div>
            <h2 class="text-base font-bold text-surface-100">Callbacks</h2>
            <p class="text-xs text-surface-500 mt-0.5">Callback provider URL</p>
          </div>
          <Button
            icon="fas fa-plus"
            label="Add"
            size="small"
            @click="openAddCallback"
          />
        </div>
        <div class="flex-1 overflow-y-auto">
          ...
          >
            ...
            <div class="col-span-8">Provider URL</div>
            <div class="col-span-1" />
          </div>
          <div
            v-for="cb in store.callbackConfigs"
            :key="cb.identifier"
          ...
          >
            ...
            <div class="col-span-1 flex justify-end">
              <Button
                ...
                size="small"
                text
                @click="onDelete(cb.identifier)"
              />
            </div>
          </div>
          <div
            v-if="store.callbackConfigs.length === 0"
            class="py-12 text-center text-surface-600 text-xs flex flex-col items-center"
          >
            <i class="fas fa-link-slash text-2xl mb-2 opacity-50" />
            <p>No callbacks configured</p>
          </div>
        </div>
      </div>
    </template>
